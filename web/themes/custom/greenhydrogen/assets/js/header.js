...
var Header = /*#__PURE__*/function () {
  function Header(context, settings, $, Drupal) {
    ...
    this.$pluses = this.$('.js-menu-plus', this.context).once(); // Search
    ...
    this.$searchCloseButton = $('.js-search-close', this.context).once(); // Mobile menu

    this.$burgerBtn = this.$('.js-header-burger', this.context).once();
    this.$mainMenu = this.$('.js-main-menu', this.context).once(); // Scrollbars
    //----------------------------------------

    this.scrollbars = []; // Events
    //----------------------------------------
    // Scroll Events

    this.prevScrollPos = this.$header.outerHeight(true);
    this.$window.on('scroll', (0, _debounce["default"])(function () {
      _this.hideOnScrollDown();
    }, 20)); // Click Events

    this.$burgerBtn.on('click', this.$.proxy(this.toggleMobileMenu, this));
    this.$pluses.on('click', $.proxy(this.toggleMenu, this)); // Search Events

    this.$searchOpenButton.on('click', $.proxy(this.openSearch, this));
    this.$searchCloseButton.on('click', $.proxy(this.closeSearch, this)); // Close search on doc click
  ...
  _createClass(Header, [{
    ...
    value: function openSearch() {
      ...
      this.$searchForm.fadeIn(this.speed, function () {
    ...
    value: function toggleMobileMenu(e) {
      e.preventDefault();
      var $elem = this.$(e.currentTarget); // Toggle open classes

      $elem.toggleClass('is-open');
      this.$mainMenu.toggleClass('is-open');
      this.$('body').toggleClass('is-scroll-locked'); // Check header class
    ...
    value: function toggleHeaderClass($elem) {
    ...
    value: function addScrollbars() {
      for (var i = 0; i < this.$mainMenu.length; i++) {
        this.scrollbars.push(new _perfectScrollbar["default"](this.$mainMenu[0]));
      }
    }
  }, {
    ...
    value: function toggleMenu(e) {
      ...
      $elem.next().slideToggle(300, function () {
        _this3.updateScrollbars();
      });
    }
  }, {
    key: "hideOnScrollDown",
    value: function hideOnScrollDown() {
      var currentScrollPos = this.$window.scrollTop();
      var prevScrollPos = this.prevScrollPos;
...
},{"../utils/debounce":3,"perfect-scrollbar":4}],3:[function(require,module,exports){
'use strict';
/**
 * Simple debouncer
 *
